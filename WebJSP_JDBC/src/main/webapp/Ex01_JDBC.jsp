...
(WEB)
 1. webapp >> WEB-INF >> lib >> jar >> 붙여넣기 >> 준비
...
<%
    ...
    out.print("false (정상연결) : " + conn.isClosed()); //false
    ...
    //실행
    ResultSet rs= stmt.executeQuery(sql); //DB서버에서 쿼리 전달하고 실해된 결과 집합의 주소 return
    ...
    //처리
    while(rs.next()){
    	out.print(rs.getInt(1) +" / " + rs.getString("dname") + " / " + rs.getString(3));
    	out.print("<br>");
    }
    ...
    out.print("true (연결종료) : " + conn.isClosed()); //true
%>  
<!DOCTYPE html>
<html>
<head>
